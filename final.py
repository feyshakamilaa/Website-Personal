...
def tambah_peserta(nama, email, instansi, no_telepon, hadir):
    ...
    # Cek apakah email atau no telepon sudah ada
...
def hapus_peserta_binary(nama_target):
    low = 0
    high = len(dataPeserta) - 1
    while low <= high:
        mid = (low + high) // 2
        if dataPeserta[mid].nama == nama_target:
            del dataPeserta[mid]
            return "Peserta berhasil dihapus."
        elif dataPeserta[mid].nama < nama_target:
            low = mid + 1
        else:
            high = mid - 1
    simpan_ke_csv()  # <--- tambahkan ini
    return "Peserta dengan nama tersebut tidak ditemukan."
...
def refresh_list():
    ...
    count = 0
    for idx, p in enumerate(dataPeserta):
